...
| BUG-0013 | 2026-09-22 | Broad `except Exception` in download/polling loops masks real bugs as transient network errors | Blanket exception handling copied across download scripts; PA-0011 sweep found the same pattern in `ebird.py` | Deferred (low priority, now 21 call sites across 7 files) — narrow to specific exception types, log traceback | OPEN |
