...
VARIANTS = {
    ...
    # Overstory canopy height / cover (distinct from EVH/EVC, which span
    # all lifeforms) - quantile-binned like the other continuous layers.
    "ch": [
        ("q2", lambda df: _qcut(df["ch"], 2)),
        ("q3", lambda df: _qcut(df["ch"], 3)),
        ("q4", lambda df: _qcut(df["ch"], 4)),
        ("q5", lambda df: _qcut(df["ch"], 5)),
    ],
    "cc": [
        ("q2", lambda df: _qcut(df["cc"], 2)),
        ("q3", lambda df: _qcut(df["cc"], 3)),
        ("q4", lambda df: _qcut(df["cc"], 4)),
        ("q5", lambda df: _qcut(df["cc"], 5)),
    ],
    # Fuel disturbance: coded categorical (disturbance type/severity/
    # time-since combos; -1 = no disturbance mapped). Raw and a coarse
    # disturbed-vs-not binary variant.
    "fdist": [
        ("raw", lambda df: df["fdist"].astype(int).astype(str)),
        ("binary", lambda df: np.where(df["fdist"].astype(float) <= 0,
                                       "None", "Disturbed")),
    ],
